<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            @if ($editTodo === $todo->id)
                <input wire:model='newName' type="text" placeholder="Todo.." class="bg-gray-100 text-gray-900 text-sm rounded block w-full p-2.5">
                @error('editTodo')
                    <span class="text-red-500 text-xs block">{{$message}}</span>
                @enderror
            @else
                <input class="mr-3" wire:click="toggle({{$todo->id}})" type="checkbox" @if ($todo->completed) checked @endif>
                @if ($todo->completed)
                    <h3 class="text-lg text-semibold text-gray-800 line-through">{{$todo->name}}</h3>
                @else
    ...
    <span class="text-xs text-gray-500">{{$todo->created_at->setTimezone('America/Sao_Paulo')->format('d/m/Y H:i:s')}} </span>
